...
class Table(object):
    ...
    def __init__(self, path: str, table_type: str, hebrew_table: bool):
        ...
        self.path = path
        self.table_type = table_type.strip()

    def delete_and_rename_columns(self, columns):
        os.remove(self.path)
        if self.table_type == 'xlsx':
    ...
    def save_changes(self):
        if self.table_type == 'xlsx':
            self.table.to_excel(self.path, index=False)
        else:
    ...
    @staticmethod
    def dataframe_to_json_list(table: pd.DataFrame) -> list[dict]:
        res = []
    ...
    def get_row_as_json_list(path: str, file_type: str, row_index: int):
        ...
        return Table.questions_row_to_json_list(row=row)
